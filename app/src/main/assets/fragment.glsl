...
void main() {
    ...
    vec3 diffL = max(dot(normal, lightL), 0.0) * srcDiffL * matDiff * v_attL;
    vec3 diffR = max(dot(normal, lightR), 0.0) * srcDiffL * matDiff * v_attR;
    vec3 diff = diffL + diffR;

    // specular term
    vec3 reflL = 2.0 * normal * dot(normal, lightL) - lightL;
    vec3 reflR = 2.0 * normal * dot(normal, lightR) - lightR;
    vec3 specL = pow(max(dot(reflL, view), 0.0), matSh) * srcSpecL * matSpec * v_attL;
    vec3 specR = pow(max(dot(reflR, view), 0.0), matSh) * srcSpecR * matSpec * v_attR;
